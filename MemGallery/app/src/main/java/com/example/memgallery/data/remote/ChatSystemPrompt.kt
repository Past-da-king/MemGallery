...
object ChatSystemPrompt {
     ...
     */
    fun generate(maxToolCalls: Int = 3): String {
...
> **⚠️ IMPORTANT:** You are limited to **$maxToolCalls tool calls maximum per request**. Plan your queries efficiently - combine filters in a single `queryDatabase` call rather than making multiple separate queries. If you need more data, ask the user to follow up.
...
2. **TOOL LIMIT**: Max $maxToolCalls calls per request.
...
""".trimIndent()
     ...
     */
    fun generateCompact(maxToolCalls: Int = 3): String {
        val now = LocalDateTime.now()
