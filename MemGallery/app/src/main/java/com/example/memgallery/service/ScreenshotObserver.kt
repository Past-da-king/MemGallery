...
class ScreenshotObserver @Inject constructor(
    ...
    private val memoryDao: com.example.memgallery.data.local.dao.MemoryDao
        ...
        uri?.let {
            ...
            scope.launch {
                if (settingsRepository.autoIndexScreenshotsFlow.first()) {
                    ...
                    // Check if memory already exists for this original URI
                    if (memoryDao.getMemoryByOriginalUri(it.toString()) != null) {
                        Log.d(TAG, "Memory already exists for URI: $it. Skipping.")
                        return@launch
                    }

                    // Check if file is pending or trashed (Android 10+)
                    if (isPendingOrTrashed(it)) {
                        Log.d(TAG, "Ignoring pending or trashed file: $it")
                        return@launch
                    }
    ...
    private fun isPendingOrTrashed(uri: Uri): Boolean {
        ...
        try {
            context.contentResolver.query(uri, projection, null, null, null)?.use { cursor ->
                if (cursor.moveToFirst()) {
                    val isPending = cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.MediaColumns.IS_PENDING)) == 1
                    val isTrashed = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        cursor.getInt(cursor.getColumnIndexOrThrow(MediaStore.MediaColumns.IS_TRASHED)) == 1
                    } else false
                    
                    return isPending || isTrashed
                }
            }
        } catch (e: Exception) {
            Log.w(TAG, "Failed to check pending/trashed status for $uri: ${e.message}")
        }
        return false
    }
