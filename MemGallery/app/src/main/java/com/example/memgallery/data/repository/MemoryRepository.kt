...
class MemoryRepository @Inject constructor(
    ...
    fun getAllCreationTimestamps(): Flow<List<Long>> {
        return memoryDao.getAllCreationTimestamps()
    }
    ...
    suspend fun savePendingMemory(
        ...
        val memoryEntity = MemoryEntity(
            ...
            originalMediaUri = imageUris.firstOrNull(), // Store the first input URI as the original source
    ...
    suspend fun deleteMemory(memory: MemoryEntity) {
        ...
        // Synchronized Deletion Logic
        try {
            val isSyncDeletionEnabled = settingsRepository.syncDeletionEnabledFlow.first()
            if (isSyncDeletionEnabled && !memory.originalMediaUri.isNullOrBlank()) {
                val originalUri = Uri.parse(memory.originalMediaUri)
                Log.d(TAG, "Attempting synchronized deletion of: $originalUri")
                
                // Only attempt to delete content:// URIs (external files)
                if (originalUri.scheme == "content") {
                    try {
                        val rows = context.contentResolver.delete(originalUri, null, null)
                        if (rows > 0) {
                            Log.d(TAG, "Successfully deleted original file: $originalUri")
                        } else {
                            Log.w(TAG, "Could not delete original file (0 rows affected). Permission might be required.")
                        }
                    } catch (e: Exception) {
                        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q && 
                            e is android.app.RecoverableSecurityException) {
                            Log.d(TAG, "Permission required for synchronized deletion of $originalUri")
                            throw e // Pass up to the ViewModel to handle UI flow
                        }
                        Log.e(TAG, "Failed to delete original file: ${e.message}")
                        // For other exceptions, we don't block app deletion
                    }
                }
            }
        } catch (e: android.app.RecoverableSecurityException) {
            // Must re-throw to allow ViewModel to handle the permission UI
            throw e
        } catch (e: Exception) {
            Log.e(TAG, "Error in synchronized deletion logic", e)
        }
    ...
    suspend fun createMemoryFromUri(uri: Uri) {
        ...
        val memoryEntity = MemoryEntity(
            ...
            audioFilePath = null,
            originalMediaUri = uri.toString(), // Store the original URI
